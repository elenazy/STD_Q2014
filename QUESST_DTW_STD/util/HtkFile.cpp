...
bool read_htk(std::string &filename, const std::string &vad_label, infra::matrix &features,bool isQuery)
{
  FILE *instream;
  ...
  int nSamples_vc = 0; // if the frame is non-speech, then that frame is not counted as samples.
  for(const auto &c:vad_label )
  {
	  if(c!='0')
		  nSamples_vc++;
  }

  features.resize(nSamples_vc, my_htk.num_coefs());
  infra::vector data(my_htk.num_coefs());

  std::string::size_type i = 0;
  int count = 0;
  int startFrame=0;
  int endFrame=10000000;
  while ( my_htk.read_next_vector(data) == my_htk.num_coefs() ) 
  {
	if(i>endFrame){
		break;
	}
	if (i < vad_label.size() && vad_label[i] != '0'&& i>=startFrame && i<=endFrame)
	{
  ...
  if(count<10){
	cout << "the frame number is "<<count<<" frame, a little short :" <<filename<<endl;
  }
  if(count!=nSamples_vc){
 	cout << "the count is not same as nSample_vc, count:" << count << ", nSamples_vc:" << nSamples_vc << "filename:"<< filename <<endl;
