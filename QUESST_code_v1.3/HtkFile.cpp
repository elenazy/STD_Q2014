...
bool read_htk(std::string &filename, const std::string &vad_label, infra::matrix &features)
{
  FILE *instream;
  ...
  {
	if (i < vad_label.size() && vad_label[i] != '0')
	{
  ...
  if(count<10){
	std::cout << "the frame number is " << count << " frame, a little short :" << filename << std::endl;
  }
  if(count!=nSamples_vc){
 	std::cout << "the count is not same as nSample_vc, count:" << count << ", nSamples_vc:" << nSamples_vc << "filename:"<< filename << std::endl;
