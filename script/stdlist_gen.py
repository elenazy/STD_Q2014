...
if __name__=='__main__':
	if len(sys.argv)<3:
		print("UASGE: query_list threshold")
		exit(0)
	querylist_filename = sys.argv[1]
	threshold = float(sys.argv[2])
	stdlistdir=sys.argv[3]
	...
	for query in querylist:
		...
		result_fid = open(query.strip()+".result")
		...
		for score in norm_scorelist:
			audio_id = 'quesst2015_'+str(audio_index).zfill(5) #let the index be in "00001" style format
			if score>threshold:
				decision="YES"
			else:
				decision="NO"
